...
export async function action({
  ...
  if (!title || !content || !["blog", "technical"].includes(category)) {
    return {
      error: "Add a title, article content, and select Blog or Technical.",
  ...
  try {
    const response = await fetch(new URL("/api/posts", request.url), {
      method: "POST",
      headers: {
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-6 py-12">
        ...
        <p className="mt-3 text-zinc-600">
          Submissions are saved as drafts and tagged for the Blog or Technical
          queue.
        </p>

        {result?.success && (
          <p className="mt-6 rounded-md bg-green-50 p-4 text-green-800">
            “{result.title}” has been sent to the queue.
          </p>
        )}

        {result?.error && (
          <p className="mt-6 rounded-md bg-red-50 p-4 text-red-800">
            {result.error}
          </p>
        )}
